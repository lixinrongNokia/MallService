...
<head>
    ...
    <style type="text/css">
        #gotoPay, #cancelOrderBtn, #createQRCode {
            display: none;
        }
    ...
    <div class="btn-bar" id="btnBar">
        <div class="bb-info" id="bbInfo">
            <div class="bottom-but">
                <a id="gotoPay" href="javascript:" onclick="openwin('pay')" class="bb-btn1-red">去支付</a>
            </div>
            <div class="bottom-but">
                <a href="javascript:" class="bb-btn1-box" id="cancelOrderBtn">
                    <div class="bb-btn1">取消订单</div>
                </a></div>
            <div class="bottom-but"><a href="javascript:" id="createQRCode" class="bb-btn1-red">生成提货码</a></div>
        </div>
    </div>
    
    <div id="qrcodeRoot" style="display: none">
        <img style="width: auto;height: auto" id="qrcode"/>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#createQRCode').click(function () {
            $('#qrcode').attr('src', 'createimgcode_pickupCode.do?tOrder.id=' + orderInfo.id);
            layer.open({content:$('#qrcodeRoot').html()});
        });
        ...
        $.getJSON("orderDetailById.do" + href, function (result) {
            if (result.success) {
                ...
                if ('等待配货' === orderInfo.orderstate && '门店自提' === orderInfo.postmethod) {
                    $('#createQRCode').css('display', 'block');
                }
