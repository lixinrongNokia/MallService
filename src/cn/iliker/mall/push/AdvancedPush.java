 ...
 */
public class AdvancedPush extends PushConfig {
    /**
     * 推送高级接口
     * <p>
     * 参见文档
     * https://help.aliyun.com/document_detail/mobilepush/api-reference/push-advanced.html
     */
    public static void PushNoticeToAllDeviceTypeAndAllDevice() throws Exception {
        ...
        pushRequest.setiOSNotificationCategory("test_category");// 设定通知Catetory(iOS
        // 10+)
        pushRequest.setiOSExtParameters("{\"attachment\":\"https://xxxx.xxx/notification_pic.png\",\"k2\":\"v2\"}"); // 自定义的kv结构,开发者扩展用
        // 针对iOS设备
        // iOS
        // 10+用attachment可以指定富媒体推送通知的资源Url
        pushRequest.setApnsEnv("DEV");
        // pushRequest.setRemind(false); // 当APP不在线时候，是否通过通知提醒
        // 推送配置: Android
        // 设置该参数后启动小米托管弹窗功能，此处指定通知点击后跳转的Activity（托管弹窗的前提条件：1. 继承小米辅助通道；2.
        // storeOffLine设为true
        // pushRequest.setXiaomiActivity("_Your_XiaoMi_Activity_");
        pushRequest.setAndroidOpenType("2"); // 点击通知后动作,1:打开应用 2: 打开应用Activity
        // 3:打开 url 4 : 无跳转逻辑
        pushRequest.setAndroidActivity("iliker.mall.LoadMap"); // Android收到推送后打开对应的url,仅仅当androidOpenType=3有效
    ...
    public void PushNoticeToDeviceList() throws Exception {
        final SimpleDateFormat dateFormat = new SimpleDateFormat("MM-dd HH:mm:ss");
        final String date = dateFormat.format(new Date());
        PushRequest pushRequest = new PushRequest();
        ...
        // storeOffLine设为true
        // pushRequest.setXiaomiActivity("_Your_XiaoMi_Activity_");
        pushRequest.setAndroidOpenType("4"); // 点击通知后动作,1:打开应用 2: 打开应用Activity
        // 3:打开 url 4 : 无跳转逻辑
        ...
        // final String pushTime = ParameterHelper.getISO8601Time(pushDate);
        // pushRequest.setPushTime(pushTime); // 延后推送。可选，如果不设置表示立即推送
        // pushRequest.setStoreOffline(false); // 离线消息是否保存,若保存,
        // 在推送时候，用户即使不在线，下一次上线则会收到
        // final String expireTime = ParameterHelper.getISO8601Time(new
        // Date(System.currentTimeMillis() + 12 * 3600 * 1000)); // 12小时后消息失效,
        // 不会再发送
     ...
     * https://help.aliyun.com/document_detail/mobilepush/api-reference/push-advanced.html
     */
    public void PushNoticeToAllDeviceTypeAndDeviceWithAccount(String accounts) throws Exception {
        final SimpleDateFormat dateFormat = new SimpleDateFormat("MM-dd HH:mm:ss");
        PushRequest pushRequest = new PushRequest();
        ...
        // account:推送给指定帐号,tag:推送给自定义标签;
        // all: 推送给全部
        pushRequest.setTargetValue(accounts); // 根据Target来设定，如Target=device,
        // 则对应的值为
        // 设备id1,设备id2.
